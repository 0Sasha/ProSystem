...
internal class CCI : Script
{
    ...
    public override void Calculate(Security symbol)
    {
        ...
        for (int i = 2; i < isGrow.Length; i++)
        {
            if (oneLevel)
            {
                if (cci[i - 1] > 0.000001) isGrow[i] = IsTrend;
                else if (cci[i - 1] < -0.000001) isGrow[i] = !IsTrend;
                else isGrow[i] = isGrow[i - 1];
            }
            else if (UseChannel)
            {
                if (cci[i - 1] - upper[i - 2] > 0.000001) isGrow[i] = IsTrend;
                else if (cci[i - 1] - lower[i - 2] < -0.000001) isGrow[i] = !IsTrend;
                else isGrow[i] = isGrow[i - 1];
            }
            else
            {
                if (cci[i - 1] - signalLine[i - 1] > 0.000001) isGrow[i] = IsTrend;
                else if (cci[i - 1] - signalLine[i - 1] < -0.000001) isGrow[i] = !IsTrend;
                else isGrow[i] = isGrow[i - 1];
            }
        }

        if (oneLevel) Result = new(ScriptType.OSC, isGrow, [cci], iBars.DateTime[^1], 0, 0, OnlyLimit);
        else if (UseChannel)
            Result = new(ScriptType.OSC, isGrow, [cci, upper, lower], iBars.DateTime[^1], OnlyLimit);
        else Result = new(ScriptType.OSC, isGrow, [cci, signalLine], iBars.DateTime[^1], OnlyLimit);
    }
}
